...
.controller('reportFormCtrl', function ($scope, $http, $compile, $sce, $rootScope, $anchorScroll, $cookies, $builder, $interpolate, $route, $validator, saveForm, $filter, $routeParams, getFormdata, $modal, $location, FormList, reportFormdata, getReportdata, getReportlist, newpatientData, savePatientdata, updatePatienttable, getsavedPatientid, getSavedreportdata, updatePatientdata, getexistingpatientData, selectinjuriesPatientdata, Patients, Injuries, $cookieStore, getExistingdata, getLatestClosedReport, currentUserData, submittedReportcount, chargeReport, getPublishReportId, getReportDataById, getReportCharge, reportFormdataByVersion, getdataClinic, reportFormdataById, getdatafromAPI, currentLoggedinUserdata, PracticeGetDataByName, deletereport, openedReportTrack, updateReportStatus, updateReportData, reportLogging, serviceworkstatusnotefax, serviceworkstatusnote, serviceGetReportsClaimAdmWise, $sessionStorage, $athenaModal) {
    ...
    $scope.newdfrReport = function () {
        ...
        // Unais' comment: hide nav bar when in report - Added check before hiding
    ...
    $scope.exitform = function (path) {
        ...
        /*Unais' comment: If condition added to cause the page to reload in case of "Save & Exit" */    
        /*if(path=='/patient/createinjury')
        {
        ...
        if ($scope.changedSections) {
            if ($scope.changedSections.length > 0) {                
				for (var i = 0; i < $scope.changedSections.length; i++) {					
					var categorypatientdata='';	
					var categoryinjurydata='';							
                    switch ($scope.changedSections[i]) {
                        case 'basicinformation':
                            categorypatientdata = $scope.basicinformation[0];
                            break;
                        case 'contactinformation':
                            categorypatientdata = $scope.contactinformation[0];
                            break;
                        case 'address':
                            categorypatientdata = $scope.address[0];
                            break;
                        case 'occupation':
                            categorypatientdata = $scope.occupation[0];
                            break;
                        case 'demographics':
                            categorypatientdata = $scope.demographics[0];
                            break;
                        case 'empduration':
                            categoryinjurydata = $scope.employment[0];
                            $scope.text = 'injury.$.injurydata.employment';
                            break;
                        case 'employer':
                            categoryinjurydata = $scope.employer[0];
                            $scope.text = 'injury.$.injurydata.employer';
                            break;
                        case 'insuranceadministrator':
                            categoryinjurydata = $scope.insurance[0];
                            $scope.text = 'injury.$.injurydata.insurance';
                            break;
                        case 'claimsadjuster':
                            categoryinjurydata = $scope.claimsadjuster[0];
                            $scope.text = 'injury.$.injurydata.claimsadjuster';
                            break;
                        case 'injuryinformation':
                            categoryinjurydata = $scope.injuryinformation[0];
                            $scope.text = 'injury.$.injurydata.injuryinformation';
                            break;
                        case 'employeraddress':
                            categoryinjurydata = $scope.employeraddress[0];
                            $scope.text = 'injury.$.injurydata.employeraddress';
                            break;
                        case 'locationaddressinjury':
                            categoryinjurydata = $scope.locationaddressinjury[0];
                            $scope.text = 'injury.$.injurydata.locationaddressinjury';
                            break;
                        case 'medicalhistory':
                            categorypatientdata = $scope.medicalhistory[0];
                            break;
                        case 'sh':
                            categorypatientdata = $scope.sh[0];
                            break;
                    }
										
                     
                    if (categoryinjurydata) {
                        Injuries.updatesubinjuries().save({ injid: $scope.injuryId, injurydata: categoryinjurydata, text: $scope.text }, function (res) {
                             
                        }).$promise.then(function () {
                            
                        })
                    }
                    if (categorypatientdata) {
                        var query = {
                            patientid: $sessionStorage.patientId,
                            category: $scope.changedSections[i],
                            categorynewdata: categorypatientdata
                        };
                                                        ...
                                                        if (err.data.message) {
                                                            if (err.data.code) {
                                                                if (err.data.code == 'E00027') {
																	...
																	//Message changed by Unais dated 26 September, 2018 to give proper message in case of expired credit card
																	$scope.popupMessage('Your credit card has expired. Please retry with a valid credit card.', 500);
                                                                }
 ...
 function checkValues(valType, val){    	
 	...
 	$scope.setPatientDataToAthena = function(){
		...
		try
		{
				...
				if(codeDetails.industrycode!='' || codeDetails.occupationcode!=''){
					...
					$scope.getAthenaOccIndustryCode(codeDetails,'updateToAthena');
				}
		}
		catch(err)
		{			
	  		...
	  		if($rootScope.currentUser.practiceDetails.isAthena){
	  			...
	  			if(typeof $scope.report.data.bginfo.firstname == 'undefined' || typeof $scope.report.data.bginfo.lastname == 'undefined' || typeof $scope.report.data.bginfo.dateofbirth == 'undefined' ){
					...
					//contactinformation
						...
						var hPhone = checkValues('PhoneNumber', $scope.report.data.bginfo.homephone);	
					  	if(hPhone !=''){
					  		query.homephone = hPhone;
					  	}					  	
					  	
					  	var mPhone = checkValues('PhoneNumber', $scope.report.data.bginfo.cellphone);
			  			if(mPhone !=''){
			  				query.mobilephone = mPhone;
			  			}
			  			
			  			var wPhone = checkValues('PhoneNumber', $scope.report.data.bginfo.workphone);
			  			if(wPhone !=''){
			  				query.workphone = wPhone;
			  			}
						
						// this comes in injury demographics
			  			var ePhone = checkValues('PhoneNumber', $scope.report.data.bginfo.emp_telephone);
			  			if(ePhone !=''){
			  				query.employerphone = ePhone;
			  			}
			    	...
			    	$http.put('/api/athena/syncpatienttoathena',query).success(function (athenaPatient, status, headers, config) {
						...
						if(codeQuery.occupationcode!='' || codeQuery.industrycode!='')
						{
			 			 ...
			 			 if(athenaPatient.updatedpatient.length>0){
			 				...
			 				if($scope.injuryData.athena_insurancepackageid == 0){
			 					...
			 					$('#isAthenaUpdateSpinner').show();
					  			...
					  			$scope.sendInsuranceDataToAthena();
			 				}
              			...
              			$http.get('/api/athena/patientmedicalsocialdata',{ params : query }
                  			...
                  			if(typeof allergypatinetdata.alleriesdata.medicalData !='undefined'){
                                 if(allergypatinetdata.alleriesdata.medicalData !='' && allergypatinetdata.alleriesdata.medicalData !=null){
                                     ...
                                     savetoPatinetCollectionAthenaPatientData('patientMedicalData','');
                                 }
                            }	                  			
                  			...
                  			if(typeof allergypatinetdata.alleriesdata.socialData !='undefined'){
                  				if(allergypatinetdata.alleriesdata.socialData !='' && allergypatinetdata.alleriesdata.socialData !=null){
									...
									savetoPatinetCollectionAthenaPatientData('patientSocialData','');		
                  				}
                  			}
    ...
    var updatefromAthenatoRatefast = function(athenaPatientData){
    	...
    	//basicinformation
			...
			if($scope.injuryData.athena_insurancepackageid == 0){
				savetoPatinetCollectionAthenaPatientData('patientData','showPopup');		
			}else{
				 $('#isAthenaUpdateSpinner').show();  				
				 $scope.athenaSpinnerMessage = 'Fetching Insurance details from Athena.';				 
				 $scope.getDetailsAthenaInsuranceData('toRatefast');
				 savetoPatinetCollectionAthenaPatientData('patientData','');
			}
													...
													if(modeOfUpdate=='toRatefast'){
															try{															
																...
																if(codeDetails.industrycode!='' || codeDetails.occupationcode!=''){
																	...
																	$scope.getAthenaOccIndustryCode(codeDetails,'updateToRatefast');
																}	
															}catch(err){
																console.log(err);
														}		
		...
		// Admin section
			...
			//claim adjuster									
				...
				$scope.report.data.bginfo.claimsadjuster_lastname = athena_injury.adjusterlastname;
				 ...
				 //Save to Database
				 savetoPatinetCollectionAthenaInjuryData('injuryData','showPopup');			
    ...
    $scope.getAthenaOccIndustryCode = function(codeInput,updateMode){    	
    	try{	    		
						...
						if(codedata.codeDetails.length>0){
							for(var i=0; i<codedata.codeDetails.length; i++){
								if(codedata.codeDetails[i].type=="occupation"){
									$scope.report.data.bginfo.jobtitle = codedata.codeDetails[i].codename;
									savetoPatinetCollectionAthenaOccupationData('occupationData','');
								}else if(codedata.codeDetails[i].type=="industry"){
									$scope.report.data.bginfo.natureofbusiness = 'Other';
					 				$scope.report.data.bginfo.othernatureofbusiness = codedata.codeDetails[i].codename;
									savetoPatinetCollectionAthenaOccupationData('industryData','');
								}		 				
							}
						}		 		
					 }).catch(function (err) {
    ...
    $scope.sendInsuranceDataToAthena = function(){
		 ...
		 try{  													  		
				...
				var adPhone = checkValues('PhoneNumber', $scope.report.data.bginfo.claimsadjuster_telephoneno);
				if(adPhone !=''){
					query.adjusterphone = adPhone;
				}
				var adFax = checkValues('PhoneNumber', $scope.report.data.bginfo.claimsadjuster_fax);
				if(adFax !=''){
                                ...
                                $rootScope.modalInstance = $modal.open({
                                     ...
                                     $rootScope.modalInstance.result.then(function (returndata) {                                           
                                         if(typeof returndata!='undefined'){
                                             ...
                                             if($scope.injuryData.athena_insurancepackageid == 0){
                                 				savetoPatinetCollectionAthenaPatientData('patientPhoneNumbers','showPopup');
												
                                 			}else{
												savetoPatinetCollectionAthenaPatientData('patientPhoneNumbers','');
                                 				 $('#isAthenaUpdateSpinner').show();  				
                                 				 $scope.athenaSpinnerMessage = 'Fetching Insurance phone numbers from Athena.';				 
                                 				 $scope.getDetailsAthenaInsuranceData('toPhoneNumber');
                                 			}
        ...
        try{          					   			 	  				 
 			 var phoneValues = {};
 				  ...
 				  if(phoneValues.employerContact =='' && phoneValues.adjusterPhone == '' && phoneValues.adjusterFax == ''){
 					  var message="<h3><p>There are no phone or fax numbers in Athena for this workers' compensation policy for patient " + patientData[0].patientid +". </p></h3>";            
 					  $scope.PopupMessageSingleButton('',message,"Okay");    
 				  }else{                                      
 					   $rootScope.modalInstance = $modal.open({
 							...
 							$rootScope.modalInstance.result.then(function (returndata) {                                           
 								if(typeof returndata!='undefined'){
 									...
 									$scope.report.data.bginfo.emp_telephone  = returndata.employerContact;
									$scope.report.data.bginfo.claimsadjuster_telephoneno  = returndata.adjusterPhone;
 									...
 									savetoPatinetCollectionAthenaInjuryData('injuryPhoneNumbers','showPopup');		
 								}
 							}, function (err) {
 								debugger;
 								// model failed
  ...
  function savetoPatinetCollectionAthenaPatientData(modeOfDataSave,isPopup){
				
				var currentUser = $rootScope.currentUser.username;
				var athenachangedSections;
				
				var basicinformation;
				var address;
				var contactinformation;
				
				if(modeOfDataSave == 'patientData'){
					
					athenachangedSections = ['basicinformation','contactinformation','address'];
				...
				if(modeOfDataSave == 'patientData' || modeOfDataSave == 'patientPhoneNumbers' ){
					
					if(modeOfDataSave == 'patientPhoneNumbers'){
						athenachangedSections = ['contactinformation'];
					}
				...
				if(modeOfDataSave == 'patientMedicalData'){
					
					athenachangedSections = ['medicalhistory'];
					
					$scope.medicalhistory = getMedicalhistoryUpdatedata(currentUser);
				}
				
				if(modeOfDataSave == 'patientSocialData'){
					
					athenachangedSections = ['sh'];
		        ...
		        if (athenachangedSections) {
		        	if (athenachangedSections.length > 0) {
		        		
		        		for (var i = 0; i < athenachangedSections.length; i++) {
		        			
		        				var categorypatientdata = '';
		        			
			        			switch (athenachangedSections[i]) {
			        				case 'basicinformation':
			        					categorypatientdata = basicinformation[0];
			        					break;
			        				case 'contactinformation':
			        					categorypatientdata = contactinformation[0];
			        					break;
			        				case 'address':
			        					categorypatientdata = address[0];
			        					break;		      
			        				case 'medicalhistory':
			        					categorypatientdata = $scope.medicalhistory[0];
			        					break;
			        			...
			        			if (categorypatientdata !='') {
			        				var query = {
			        					patientid: $sessionStorage.patientId,
			        					category: athenachangedSections[i],
			        					categorynewdata: categorypatientdata
			        				};
			        			...
			        			if(isPopup=='showPopup'){
			        				if(athenachangedSections.length-1 == i){
    ...
    function savetoPatinetCollectionAthenaInjuryData(modeOfDataSave,isPopup){
    	
		var currentUser = $rootScope.currentUser.username;
		var athenachangedSections = [];
		
		if(modeOfDataSave == 'injuryData'){
			
			athenachangedSections = ['employer','insuranceadministrator','claimsadjuster','employeraddress'];
		...
		if(modeOfDataSave == 'injuryData' || modeOfDataSave == 'injuryPhoneNumbers' ){
			
			if(modeOfDataSave == 'injuryPhoneNumbers'){
				athenachangedSections = ['employer', 'claimsadjuster'];
			}
        			...
        			switch (athenachangedSections[i]) {	        				
	        			...
	        			if(isPopup=='showPopup'){
	        				if(athenachangedSections.length-1 == i){
			    				...
			    				if(modeOfDataSave == 'injuryPhoneNumbers'){
			    				  message="<h3><p>Patient and Insurance phone numbers were synced successfully.</p></h3>";
			    				}
    ...
    function savetoPatinetCollectionAthenaOccupationData(modeOfDataSave,isPopup){
    	
		var currentUser = $rootScope.currentUser.username;
		var athenachangedSections = [];
		
		if(modeOfDataSave == 'industryData'){
			
			athenachangedSections = ['employer'];
		...
		}else if(modeOfDataSave == 'occupationData'){
			
			athenachangedSections = ['empduration'];
